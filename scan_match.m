...
[tform,ptXYZ2r] = pcregrigid(pts2,pts1);
...
A=[R,[0 0 0]';T];
tfr=affine3d(A);
pts2r=pctransform(pts2,tfr);
%%
tic;
[ptcloud2,nptcloud2,plane2]=groundPlane(pts2r,4);
% [xyz2,nxyz2,gr2,ngr2,pr2,plane2,Vr2]=ransac_plane(ptXYZ2r.Location,ptXYZ2r.Normal,ptXYZ2r.Count,10);
% ptcloud2=pointCloud(xyz2,'Normal',gr2);
% nptcloud2=pointCloud(nxyz2,'Normal',ngr2);
%%
[R12,eul]=find_rotation(plane1,plane2);
t=-mean(pts1.Location)*R12'+mean(pts2r.Location);
B=[R12,[0 0 0]';[t,1]];
tf12=affine3d(B);
pts2rr=pctransform(pts2r,tf12);
nptcloud2r=pctransform(nptcloud2,tf12);
%%
[tform1,nptcloud2rt] = pcregrigid(nptcloud2r,pts1);
pts22r = pctransform(pts2rr,tform1);
tf1=pcregrigid(pts22r,pts1);t1(i,1)=toc;
tic;[tform2,pts2rt]=pcregrigid(pts2r,pts1);t2(i,1)=toc;
tf2=pcregrigid(pts2rt,pts1);
...
%%
% plot_plane_fit(pts1.Location,ptcloud1.Location,plane1.Normal',mean(pts1.Location),ptcloud1.Normal,1);
% plot_plane_fit(pts2r.Location,ptcloud2.Location,plane2.Normal',mean(pts2r.Location),ptcloud2.Normal,1);
% plot_plane_fit(pts2r.Location,xyz,Nr,pr,gr,3);
figure(1)
